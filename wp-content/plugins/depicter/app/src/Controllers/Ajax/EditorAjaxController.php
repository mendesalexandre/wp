...
class EditorAjaxController implements RestMethodsInterface
{
	...
	public function store( RequestInterface $request, $view ){
		...
		// Get editor data
		$editor = Sanitize::textarea( $request->body( 'editor' ) );
		...
		try{
			if( $properties['status'] == 'publish' ){
				if( function_exists( 'get_filesystem_method' ) && get_filesystem_method() != 'direct' ){
					throw new Exception( esc_html__( 'Media files cannot be published due to lack of proper file permissions for uploads directory.', PLUGIN_DOMAIN ) );
				}
	...
	public function setDocumentPoster( RequestInterface $request, $id ){
		...
		return Depicter::storage()->filesystem()->write( Depicter::storage()->uploads()->getBaseDirectory() . '/depicter/preview-images/' . $id . '.' . $extension, (string) $coverPhoto->getStream() );
	}
