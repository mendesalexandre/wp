...
class Element
{
	...
	public function getLinkTag() {
		if ( !empty( $this->options->url->path ) ) {
			$urlArgs = !empty( $this->options->url->openInNewTab ) ? ['target' => '_blank'] : [];
			return Html::a( '', $this->options->url->path, $urlArgs );
		}
	...
	public function getCssID() {
		return Selector::getFullSelectorPath( $this->getDocumentID(), null, $this->getID() );
	}
