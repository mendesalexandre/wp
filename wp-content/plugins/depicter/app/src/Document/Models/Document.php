...
class Document implements HydratableInterface
{
	...
	protected function getSelectorAndCssList(){
		...
		$documentStyles = [
			...
			'.'. $this->getStyleSelector() . ':not(.depicter-ready)' => $this->options->getBeforeInitStyles(), // styles to prevent FOUC
	 ...
	 *
	 * @param bool $reindexStyles
	 *
	 * @return StyleGeneratorService
	 */
	public function styleGenerator( $reindexStyles = false ){
		if( ! $this->styleGenerator ){
			$this->styleGenerator =  new StyleGeneratorService( $this->stylesList, $this->getDocumentID() );
		}
		if( $reindexStyles ){
	...
	public function saveCss( $forceRegenerateStyles = false ){
		$this->styleGenerator( $forceRegenerateStyles )->saveCss( $forceRegenerateStyles = false );
	}
	...
	public function getCss( $forceRegenerateStyles = false ){
		return $this->styleGenerator( $forceRegenerateStyles )->getCss( $forceRegenerateStyles = false );
	}
	...
	public function getInlineCssTag( $forceRegenerateStyles = false ){
		return $this->styleGenerator( $forceRegenerateStyles )->getCssAndTag( $forceRegenerateStyles = false );
	}
	...
	protected function getClassNames() {
		$classes = [ Selector::PREFIX_NAME, Selector::prefixify( Selector::DOCUMENT_PREFIX ) ];
