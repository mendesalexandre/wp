		...
		foreach ( $devices as $device ) {
			if ( isset( $this->{$device}->enable ) ) {
				...
				if( $device !== 'default' && ! empty( $this->default->enable )  && ! $this->{$device}->enable ) {
					...
					$this->contrast = !empty($this->{$device}->contrast) ? $this->{$device}->contrast : $this->contrast;
					$this->saturation = !empty($this->{$device}->saturation) ? $this->{$device}->saturation : $this->saturation;
					$this->hue = !empty($this->{$device}->hue) ? $this->{$device}->hue : $this->hue;
