...
class SystemCheckService {
	...
	public function system_status_check( $tests ) {
		...
		$tests['direct']['depicter_curl_check'] = array(
			'label' => __( 'Check to access depicter resources servers.', PLUGIN_DOMAIN ),
	...
	public function check_server_connection() {
		$result = array(
			'label'       => __( 'Connecting to depicter resources servers passed successfully', PLUGIN_DOMAIN ),
			'status'      => 'good',
			'badge'       => array(
			...
			'description' => __( 'Connecting to depicter resources servers passed successfully', PLUGIN_DOMAIN )
		);
		...
		try {
			...
			if ( $response->getStatusCode() != 200 ) {
				...
				$result['label'] = __( 'Error while trying to connect to depicter resources servers', PLUGIN_DOMAIN );
			}
	...
	public function getErrorOuput() {
		$screen_reader = __( 'Error', PLUGIN_DOMAIN );
		$message       = __( 'Your site cannot communicate securely with depicter services. Contact your host provider and ask them to whitelist <code>depicter.com</code> to gain access to assets library', PLUGIN_DOMAIN );
